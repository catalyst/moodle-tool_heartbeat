...
if ($hassiteconfig) {
    ...
    if (!during_initial_install()) {
        ...
        $settings->add(new admin_setting_configduration(
            'tool_heartbeat/errorlog',
            get_string('errorlog', 'tool_heartbeat'),
            get_string('errorlogdesc', 'tool_heartbeat'),
            30 * MINSECS,
            MINSECS
        ));

        $settings->add(new admin_setting_configtext(
            'tool_heartbeat/configuredauths',
            get_string('configuredauths', 'tool_heartbeat'),
            get_string('configuredauthsdesc', 'tool_heartbeat'),
            '',
            PARAM_TEXT
        ));
        ...
        $settings->add(new admin_setting_configselect(
            'tool_heartbeat/errorcritical',
            get_string('errorascritical', 'tool_heartbeat'),
            get_string('errorascritical_desc', 'tool_heartbeat', $time->format('e P')),
            'warning',
            $opts
        ));

        $settings->add(new admin_setting_configduration(
            'tool_heartbeat/mutedefault',
            get_string('settings:mutedefault', 'tool_heartbeat'),
            get_string('settings:mutedefault:desc', 'tool_heartbeat'),
            12 * WEEKSECS,
            WEEKSECS
        ));
        ...
        $settings->add(new admin_setting_configselect(
            ...
            get_string('settings:mutedefaultstatus:desc', 'tool_heartbeat'),
            key($statuslist),
            $statuslist
        ));

        $settings->add(new admin_setting_configtext(
            'tool_heartbeat/muteurlregex',
            get_string('settings:muteurlregex', 'tool_heartbeat'),
            get_string('settings:muteurlregex:desc', 'tool_heartbeat'),
            '',
            PARAM_TEXT
        ));
        ...
        $settings->add(new admin_setting_configtextarea(
            'tool_heartbeat/tasklatencymonitoring',
        ...
        $settings->add(new admin_setting_heading(
            'tool_heartbeat/cachechecksettings',
            get_string('settings:cachecheckheading', 'tool_heartbeat'),
            ''
        ));

        $settings->add(new admin_setting_configcheckbox(
            'tool_heartbeat/shouldlogcacheping',
        ...
        $settings->add(new admin_setting_configcheckbox(
            'tool_heartbeat/shouldlogcachecheck',
